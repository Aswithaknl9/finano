    ...
    <div className='bg-[#f3f4f6]'>
      <div className="container mx-auto mt-16 pt-[85px] px-[15px] pb-[75px]">
        <div className="text-center mb-8">
          <div className="ct-heading-sub text-[#ec0203] font-semibold text-sm uppercase mb-4">Our Dedicated Services</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {services.map((service, index) => (
            <div
              key={index}
              className="grid-item bg-white border p-6 flex flex-col items-center hover:bg-[#ec0203] hover:text-white hover:shadow-xl">
            
              <div className="item-icon text-gradient text-4xl mb-4">
                <i className={`fa ${service.icon}`}></i>
              </div>
              <h3 className="item-title text-xl font-semibold mb-2">{service.title}</h3>
              <p className="item-except text-sm text-gray-500 mb-4">{service.description}</p>
            </div>
          ))}
        </div>
      </div>
